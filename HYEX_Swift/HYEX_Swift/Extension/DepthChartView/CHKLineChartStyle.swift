...
/**
 ...
 - None:  不显示
 */
@objc enum CHYAxisShowPosition : NSInteger {
    case left
    case right
    case none
}
...
// MARK: - 图表样式配置类
@objc open class CHKLineChartStyle : NSObject {
    /// 背景颜色
    ...
    /**
     ...
     - returns:
     */
    @objc open var padding: UIEdgeInsets = UIEdgeInsets.init(top: 0, left: 0, bottom: 0, right: 0)
    
    //字体大小
    @objc open var labelFont: UIFont!
    
    //线条颜色
    @objc open var lineColor: UIColor = UIColor.clear
    
    //文字颜色
    @objc open var textColor: UIColor = UIColor.clear
    ...
    //显示y的位置，默认右边
    @objc var showYAxisLabel = CHYAxisShowPosition.right
    
    /// 是否把y坐标内嵌到图表仲
    @objc open var isInnerYAxis: Bool = false
    ...
    /// 买单居右
    var bidChartOnDirection = CHKDepthChartOnDirection.right
